...
import com.indeed.imhotep.shardmanager.rpc.RequestResponseClient;
...
public class ShardManagerDaemonTest {
    @Rule
    ...
    public void testIt() throws IOException, InterruptedException, ExecutionException, KeeperException {
        ...
        final ShardManagerDaemon.Config config = new ShardManagerDaemon.Config()
        ...
        final ShardManagerDaemon shardManagerDaemon = new ShardManagerDaemon(config);
        final Thread daemonRunner = new Thread(new Runnable() {
            @Override
            public void run() {
                try {
                    shardManagerDaemon.run();
