...
public abstract class AbstractImhotepServiceCore
    ...
    @Override
    public void handleGetFTGSIteratorSplit(final String sessionId, final String groupsName, final String[] intFields, final String[] stringFields, final OutputStream os, final int splitIndex, final int numSplits, final long termLimit, @Nullable final List<List<String>> stats) throws IOException, ImhotepOutOfMemoryException {
        doWithSession(sessionId, (IOOrOutOfMemoryFunction<MTImhotepLocalMultiSession, Void>) session -> {
    ...
    @Override
    public void handleGetSubsetFTGSIteratorSplit(final String sessionId, final String groupsName, final Map<String, long[]> intFields, final Map<String, String[]> stringFields, @Nullable final List<List<String>> stats, final OutputStream os, final int splitIndex, final int numSplits) throws IOException, ImhotepOutOfMemoryException {
        doWithSession(sessionId, (IOOrOutOfMemoryFunction<MTImhotepLocalMultiSession, Void>) session -> {
    ...
    @Override
    public int handleRegroup(final String sessionId, final RegroupParams regroupParams, final GroupRemapRule[] remapRules) throws ImhotepOutOfMemoryException {
        return doWithSession(sessionId, (ThrowingFunction<MTImhotepLocalMultiSession, Integer, ImhotepOutOfMemoryException>) session -> session.regroup(regroupParams, remapRules));
    }

    public int handleRegroup(final String sessionId, final RegroupParams regroupParams, final int numRemapRules, final Iterator<GroupRemapRule> remapRules) throws ImhotepOutOfMemoryException {
        return doWithSession(sessionId, (ThrowingFunction<MTImhotepLocalMultiSession, Integer, ImhotepOutOfMemoryException>) session -> session.regroup2(regroupParams, numRemapRules, remapRules));
    }
