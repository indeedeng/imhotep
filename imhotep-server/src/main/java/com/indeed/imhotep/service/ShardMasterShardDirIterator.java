...
public class ShardMasterShardDirIterator implements  ShardDirIterator {
    private final Supplier<ShardMaster> shardMaster;
    private final String node;

    public ShardMasterShardDirIterator(final Supplier<ShardMaster> shardMaster, final String node) {
    ...
    public Iterator<Pair<String, ShardDir>> iterator() {
        ...
        try {
            assignments = shardMaster.get().getAssignments(node);
