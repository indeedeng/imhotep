...
public enum ImhotepConnectionPool implements Closeable {
    ...
    /**
     * Execute the function with the connection, also set the timeout when getting connection
     * But the connection won't be closed atomically, clients should close it whenever finishing the usage.
     * The method is helpful if clients want to read from the socket later.
     */
    public <R, E extends Exception> R withConnectionManuallyClose(
            final Host host,
            final int timeoutMillis,
            final ThrowingFunction<ImhotepConnection, R, E> function) throws E, IOException {
        final ImhotepConnection connection = getConnection(host, timeoutMillis);
        try {
            return function.apply(connection);
        } catch (final Throwable t) {
            connection.markAsInvalid();
